 ...
 *
 *  Kidde/Ecolink Z-Wave Smoke Detector Relay
 *    (recycles code from Z-Wave Door/Window Sensor and Z-Wave Smoke Alarm device handlers)
 *
...
// for the UI
metadata {
    definition (name: "Kidde/Ecolink Smoke Detector Relay", namespace: "tomriv77", author: "tomriv77") {
    ...
    simulator {
        ...
        status "50 percent battery":
            zwave.batteryV1.batteryReport(batteryLevel:0x32).incomingMessage()
        status "100 percent battery":
            zwave.batteryV1.batteryReport(batteryLevel:0x64).incomingMessage()
    }
    ...
    tiles(scale: 2) {
            ...
            //        attributeState "detected", label:'${name}', icon:"st.alarm.smoke.smoke", backgroundColor:"#ff0000"
            //    }
            //}

            standardTile("smoke", "device.smoke", width: 3, height: 3) {
                state "clear", label:'${name}', icon: "st.alarm.smoke.clear", backgroundColor: "#79b821"
                state "detected", label:'${name}', icon: "st.alarm.smoke.smoke", backgroundColor: "#ff0000"
            }
            ...
            standardTile("refresh", "device.refresh", inactiveLabel: false, width: 3, height: 3) {
